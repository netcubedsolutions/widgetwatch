...
describe('normalizeFlightNumber (FR24)', () => {
  it('prepends DL to bare numbers', () => {
    expect(normalizeFlightNumber('838')).toBe('DL838');
  });

  it('converts DAL prefix to DL', () => {
    expect(normalizeFlightNumber('DAL838')).toBe('DL838');
  });

  it('leaves DL prefix as-is', () => {
  ...
  it('handles 4-digit flight numbers', () => {
    expect(normalizeFlightNumber('2221')).toBe('DL2221');
  });

  it('preserves non-DL airline codes', () => {
...
describe('normalizeLiveResponse', () => {
  it('maps FR24 live fields to standard schema', () => {
    ...
    const result = normalizeLiveResponse(data, 'DL838');

    expect(result.flightNumber).toBe('DL838');
    expect(result.callsign).toBe('DAL838');
  ...
  it('returns "on-ground" status when on_ground is true', () => {
    ...
    const result = normalizeLiveResponse(data, 'DL100');
    expect(result.status).toBe('on-ground');
  });

  it('returns null for empty data', () => {
    expect(normalizeLiveResponse({ data: [] }, 'DL100')).toBeNull();
    expect(normalizeLiveResponse({}, 'DL100')).toBeNull();
    expect(normalizeLiveResponse(null, 'DL100')).toBeNull();
  });
  ...
  it('uses fallback fields when primary fields are missing', () => {
    ...
    const result = normalizeLiveResponse(data, 'DL838');
    expect(result.flightNumber).toBe('DAL838');
    expect(result.origin.iata).toBe('SFO');
...
describe('normalizeSummaryResponse', () => {
  it('maps FR24 summary fields to standard schema', () => {
    ...
    const result = normalizeSummaryResponse(data, 'DL1234');

    expect(result.flightNumber).toBe('DL1234');
    expect(result.callsign).toBe('DAL1234');
  ...
  it('returns null for empty data', () => {
    expect(normalizeSummaryResponse({ data: [] }, 'DL100')).toBeNull();
    expect(normalizeSummaryResponse({}, 'DL100')).toBeNull();
    expect(normalizeSummaryResponse(null, 'DL100')).toBeNull();
  });

  it('uses fallback fields when primary fields are missing', () => {
    const data = {
      data: [{
        flight_number: { iata: 'DL999', icao: 'DAL999' },
        airport: {
    ...
    const result = normalizeSummaryResponse(data, 'DL999');
